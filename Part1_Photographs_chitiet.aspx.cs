...
public partial class Part1_Photographs_chitiet : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string sql_hinhanh = "select A.IDCauhoi,C.IDNoidungchitiet, A.Phancauhoi, RTRIM(B.Tenfile)as Tenfile,A.Noidung from CAUHOI A, FILES B, CHITIETNOIDUNG C where A.IDNoidung='Photographs' and C.IDNoidungchitiet=A.IDNoidung and A.IDFile=B.IDFile and A.Made='"+id+"'order by A.IDCauhoi ASC";
            ...
            for (int i = 0; i < Rpt_nghe_hinhanh.Items.Count; i++)
            {
                ...
                string sql = "select * from DAPAN where IDCauhoi ='1_Pho_"+(i+1)+"'";
                try
                {
            ...
            try
            {
                string sql_filenghe = "select * from FILES where IDFile='Nghe_Photo_" + id.Substring(0, 2) + "'";
                DataTable tb_filenghe = ketnoi.getData(sql_filenghe);
    ...
    protected void audio_click(object sender, EventArgs e)
    {
        Response.Write("<script> alert('Bạn không được phép nghe lại!')</script>");
    }
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < Rpt_nghe_hinhanh.Items.Count; i++)
        {
            string sql_dapan = "select * from CAUHOI where IDNoidung='Photographs'";
